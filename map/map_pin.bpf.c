#include <linux/bpf.h>
#include <bpf/bpf_helpers.h>

char LICENSE[] SEC("license") = "Dual BSD/GPL";

struct {
    __uint(type, BPF_MAP_TYPE_HASH);
    __type(key, int);
    __type(value, int);
    __uint(max_entries, 1);
    __uint(pinning, LIBBPF_PIN_BY_NAME);
} map_kern SEC(".maps");

SEC("tracepoint/syscalls/sys_enter_execve")
int handle_tracepoint(void *ctx) {
    int key = 321;
    int *value = bpf_map_lookup_elem(&map_kern, &key);
    if (!value) {
        bpf_printk("failed to lookup for key(%d) in eBPF map \'map_kern\'. Trying to insert a new value...", key);
        int new_value = 5;
        int result = bpf_map_update_elem(&map_kern, &key, &new_value, BPF_NOEXIST);
        if (result < 0) {
            bpf_printk("failed to insert new value(%d) for key(%d) in eBPF map \'map_kern\'", new_value, key);
            return -1;
        }
        bpf_printk("inserted new value(%d) for key(%d) in eBPF map \'map_kern\'", new_value, key);
        return 0;
    }
    return 0;
}
